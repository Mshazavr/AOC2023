...
struct Hand { 
    ...
    HandType compute_hand_type() {
        ...
        if (num_quadruples == 1) return HandType::FourOfAKind;
        if (num_triples == 1 && num_pairs == 1) return HandType::FullHouse;
        if (num_triples == 1) return HandType::ThreeOfAKind;
        if (num_pairs == 2) return HandType::TwoPairs;
        if (num_pairs == 1) return HandType::OnePair;
